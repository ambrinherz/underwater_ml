...
def get_avg_dist(results):
	...
	for row in results:
		...
		dist1 = abs(actual - float(row[6]))
		dist2 = abs(actual - float(row[7]))
...
def leroy_evaluation(row):
	...
	result = 1402.5 + 5 * temp - .0544 * temp ** 2 + .00021 * temp ** 3 + 1.33 * sal - .0123 * sal * temp + .000087 * sal * temp ** 2 + .0156 * depth + .000000255 * depth ** 2 - .0000000000073 * depth ** 2 + .0000012 * depth * (lat - 45) - .00000000000095 * temp * depth ** 3 + .0000003 * temp ** 2 * depth + .0000143 * sal * depth
	#print(result)
	return result
...
if __name__ == '__main__':
	evaluate(
		'worldwide/ll_worldwide_sample_30.csv',
		'worldwide-ll-decisionTree-1606255008.7046468.model',
		'worldwide-ll-decisionTree-1606255007.8135018.scalar',
		'worldwide/results-ll-worldwide-decisionTree')
